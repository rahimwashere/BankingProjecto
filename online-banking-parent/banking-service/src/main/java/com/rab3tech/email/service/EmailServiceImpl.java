...
public class EmailServiceImpl implements EmailService{
	...
	@Override
	@Async("threadPool")
	public String sendEditPayeeEmail(EmailVO mail) {
MimeMessage message = javaMailSender.createMimeMessage();
try {
    MimeMessageHelper helper = new MimeMessageHelper(message,
            MimeMessageHelper.MULTIPART_MODE_MIXED_RELATED,
    ...
    Map<String,Object> props=new HashMap<>();
    props.put("name", mail.getName());
    props.put("payeeName", mail.getName());
    props.put("payeeAccountNo", mail.getBody());
    props.put("sign", "Banking Application");
    props.put("location", "fremont CA100, USA");
    props.put("mail", "shahzadladiwala123@gmail.com");
    context.setVariables(props);
    String html = templateEngine.process("enquiry-email-template", context);
